    ...
    public class DuelEffectProcessor : IPlayedCardEffectProcessor
    {
        ...
        public MoveResult ProcessCardEffect(Game game, MoveParams moveParams, string messageToLog)
        {
            ...
            var callerWon = moveParams.DuelNumbers.Contains(numberRolled);
             messageToLog += $"{moveParams.PlayerPlayed.User.Name} targeted {moveParams.PlayerTargeted.User.Name} with Duel. Numbers they picked: {String.Join(' ', moveParams.DuelNumbers)}. Number rolled: {numberRolled}. ";
