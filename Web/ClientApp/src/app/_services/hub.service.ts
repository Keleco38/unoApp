...
export class HubService {
  ...
  private _updateSpectatorsViewHandsAndUserObservable = new Subject<
  ...
  private _updateRetrieveFullGameChatMessagesObservable = new Subject<
    ChatMessage[]
  >();
  private _updateRetrieveFullTournamentChatMessagesObservable = new Subject<
    ChatMessage[]
  >();
  ...
  constructor(
    ...
    this._hubConnection.on("UserMentioned", () => {
      var notifyWhenMentionedToast = this._utilityService.userSettings
        .notifyWhenMentionedToast;
      var notifyWhenMentionedBuzz = this._utilityService.userSettings
        .notifyWhenMentionedBuzz;
    ...
    this._hubConnection.on(
      "PostNewMessage",
      (message: ChatMessage, chatDestination: ChatDestination) => {
    ...
    this._hubConnection.on(
      "RetrieveFullChat",
      (messages: ChatMessage[], chatDestination: ChatDestination) => {
        if (chatDestination == ChatDestination.tournament) {
          this._updateRetrieveFullTournamentChatMessagesObservable.next(
            messages
          );
    ...
    this._hubConnection.on(
      "DisplayToastMessage",
      (message: string, toastrType: string) => {
    ...
    this._hubConnection.on(
      "UpdateSpectatorsViewHandsAndUser",
      (spectatorsView: KeyValue<string, Card[]>[]) => {
    ...
    this._hubConnection.on("AdminKickUser", () => {
      ...
      document.body.innerHTML =
        "<h1>You were temporarily kicked from the server</h1>";
    });

    this._hubConnection.on(
      "ShowCards",
      (cardsAndNames: KeyValue<string, Card[]>[], showImmediately: boolean) => {
    ...
    this._hubConnection.on("UpdateTournament", (tournament: Tournament) => {
      ...
      if (tournament.tournamentStarted) {
        if (
          this._router.url !== "/tournament" &&
          this._router.url !== "/game"
        ) {
          this._router.navigateByUrl("/tournament");
        }
      } else {
  ...
  private buzzPlayer(buzzType: string, forceBuzz: boolean) {
    var index = this._utilityService.userSettings.blockedBuzzCommands.indexOf(
      buzzType
    );
    if (forceBuzz) {
      index = -1;
